...
def get_docker_ip(container : str):
    byte_host_ip = subprocess.check_output(f'ping {container}' + ' -c1 | head -1 | grep -Eo "[0-9.]{4,}"', shell=True)
    return  byte_host_ip.decode('utf-8').replace('\n', "")

def get_launch(launch_file : str, host_ip : str):
